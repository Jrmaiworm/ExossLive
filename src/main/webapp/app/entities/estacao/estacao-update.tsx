...
import { getEntities as getCapturas } from 'app/entities/captura/captura.reducer';
...
export const EstacaoUpdate = (props: RouteComponentProps<{ id: string }>) => {
  ...
  const capturas = useAppSelector(state => state.captura.entities);
  ...
  const saveEntity = values => {
    const entity = {
      ...
      captura: capturas.find(it => it.id.toString() === values.captura.toString()),
    };
  ...
  const defaultValues = () =>
    isNew
      ...
      : {
          ...
          captura: estacaoEntity?.captura?.id,
        };
  ...
  return (
    <div>
      <Row className="justify-content-center">
        <Col md="8">
          <h2 id="exossLiveApp.estacao.home.createOrEditLabel" data-cy="EstacaoCreateUpdateHeading">
        ...
        <Col md="8">
          {loading ? (
            ...
            <ValidatedForm defaultValues={defaultValues()} onSubmit={saveEntity}>
              ...
              <ValidatedField id="estacao-captura" name="captura" data-cy="captura" label="Captura" type="select">
                <option value="" key="0" />
                {capturas
                  ? capturas.map(otherEntity => (
                      <option value={otherEntity.id} key={otherEntity.id}>
                        {otherEntity.id}
                      </option>
                    ))
